...
type Intent struct {
	...
	// populated for query intents (structured query, replaces reply_text for data lookups)
...
// QueryData describes what real-data lookup is needed for a query intent.
type QueryData struct {
	// "stock_by_code" | "top_stock" | "price_by_code" |
	// "recent_timbangan" | "recent_expenses" | "recent_beli_ikan" | "dashboard" | "general"
	QueryType string `json:"query_type"`
	// for stock_by_code and price_by_code: the fish code asked about
	FishCode string `json:"fish_code,omitempty"`
	// for top_stock / recent queries: how many results (default 5)
