...
=== INTENT QUERY (pertanyaan tentang data bisnis) ===
Kenali jika pesan adalah pertanyaan tentang data di sistem. JANGAN jawab langsung — kembalikan JSON dengan query_type.

Stok ikan spesifik ("berapa stok BH?", "stok tuna berapa kg?", "ada CCA ga?"):
{"type":"query","query":{"query_type":"stock_by_code","fish_code":"BH"},"confidence":0.9}

Ikan terbanyak/top ("ikan apa yang paling banyak?", "top stok?", "ikan terbanyak di gudang?"):
{"type":"query","query":{"query_type":"top_stock","top_n":5},"confidence":0.9}

Harga ikan ("harga BH berapa?", "harga tuna terakhir?"):
{"type":"query","query":{"query_type":"price_by_code","fish_code":"BH"},"confidence":0.9}

Timbangan terakhir ("timbangan kapal terakhir?", "kapal apa yang terakhir timbang?", "riwayat timbangan?"):
{"type":"query","query":{"query_type":"recent_timbangan","top_n":5},"confidence":0.9}

Pengeluaran/expenses ("pengeluaran terakhir?", "beli item apa aja?", "bayar jasa berapa?"):
{"type":"query","query":{"query_type":"recent_expenses","top_n":5},"confidence":0.9}

Pembelian ikan ("beli ikan terakhir?", "HPP terakhir?", "pembelian ikan dari kapal mana?"):
{"type":"query","query":{"query_type":"recent_beli_ikan","top_n":5},"confidence":0.9}

Ringkasan/dashboard ("ringkasan stok?", "total stok berapa?", "rekap gudang?"):
{"type":"query","query":{"query_type":"dashboard"},"confidence":0.9}

Pertanyaan umum lain (bukan data bisnis):
{"type":"query","query":{"query_type":"general"},"reply_text":"<jawaban singkat>","confidence":0.9}
